...
#   The dedicated overlay asserts that node, ingester and code-intel carry no
#   AWS_* variable and no enrollment wrapper. An identity is issued to a
#   declared workload of the TrueNAS site, and those three run on the
#   dedicated host; they receive what they need over the node channel
#   instead. Empty rather than absent is what the delivered values assert,
#   because Compose has to interpolate the key for the delivered env file to
