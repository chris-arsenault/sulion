...
validate-deploy:
	...
	test "$$(docker compose --env-file deploy/compose.validate.env -f compose.yaml config --services | sort | xargs)" = "backend broker frontend node-tunnel retrieval"
	...
	docker compose --env-file deploy/compose.validate.env -f compose.yaml config --format json | jq -e '.services.backend.image == "ghcr.io/chris-arsenault/sulion/backend:test" and .services.backend.environment.SULION_DEPLOYMENT_ROLE == "control-plane" and .services.backend.environment.SULION_NODE_TRANSPORT == "remote" and .services.backend.environment.SULION_DOCKER_MODE == "none" and .services.backend.environment.SULION_NODE_LAN_CIDR == "192.168.66.0/24" and .services.backend.environment.SULION_NODE_TRUSTED_PROXY_CIDR == null and (.services.backend.volumes // []) == [] and (.services.backend.ports | length) == 2 and any(.services.backend.ports[]; .target == 8080 and .published == "30081" and .host_ip == "192.168.66.3") and (.services.frontend.ports | length) == 1 and .services.frontend.ports[0].target == 80 and .services["code-intel"] == null and .services.runner == null and .services.frontend.environment.SULION_RETRIEVAL_UPSTREAM == "retrieval:8083" and .services["node-tunnel"].network_mode == "service:backend" and (.services["node-tunnel"].cap_add | index("NET_ADMIN")) != null and (.services.backend.cap_add // []) == [] and any(.services.backend.ports[]; .target == 51820 and .protocol == "udp") and .services.backend.environment.SULION_TUNNEL_ENDPOINT == "192.168.66.3:51820"'
