...
pub struct AppState {
    ...
    /// Durable development-node registry and authenticated transport. Local
    /// mutations and terminal attachment always cross it; standalone differs
    /// only in that its node runs in this process over loopback.
