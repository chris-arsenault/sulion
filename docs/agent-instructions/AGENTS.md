...
## CRITICAL — File edits go through apply-patch. Never through the shell.
...
Every modification to an existing file, and every new file, goes through the
default apply-patch/editor tooling.
...
acceptable. If a change spans thirty files, patch thirty files.

These are rationalizations, not exceptions — reject all of them:

- "It is a one-line change." Patch it.
- "It is the same replacement across several files." Patch each file.
- "Exact-match keeps failing." Re-read the file and match the real text.
- "I am only appending." Patch, anchored on the file's current tail.
- "It is a scratch or throwaway file." Patch it.
...
The broker lives at `/opt/sulion/bin/with-cred`. It injects secrets as env vars for **that one command only** and keeps them out of shell startup files, `.env` files, and logs.

```bash
with-cred -- curl -H "x-key: $BFL_API_KEY" https://api.bfl.ai/...
with-cred bfl-key -- node dist/cli.js ...  # request a specific secret by id
with-cred aws-default -- aws s3 ls         # (the pre-wrapped `aws` shim also works directly)
```
...
- If a command fails with a missing-env-var error, re-run it as `with-cred -- <command>` before trying any other fix.
- The broker denying access exits `66` and names the refused `(pty, tool, secret_id)` on stderr; surface it so the user can grant the credential.
- `$SULION_PTY_ID` confirms you're in a managed PTY; `$SULION_SECRET_BROKER_URL` is the broker endpoint.
- The pre-wrapped `aws` shim (at `/opt/sulion/bin/aws`, earlier on PATH than system aws) already wraps `with-cred aws-default -- /usr/bin/aws ...`, so `aws s3 ls` works directly.
