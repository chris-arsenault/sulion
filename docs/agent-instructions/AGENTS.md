...
publication in the current request. A private default, a table-only result, or
easier viewing is not authorization. "On the screen" means markdown in chat;
put oversized output in a local file and give me its path.
...
- **Add `--include user` to find something I said.** The default search returns
  ...
  `~/.codex/sessions/**/rollout-*.jsonl`; run `sulion-retrieve search
  "<my words>" --include user --mode lexical` instead.
