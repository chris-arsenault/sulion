...
| Meta-repository organization and collection sessions | Covered | High | `09` creates a real metadata grouping from seeded repos, verifies the nested sidebar and rail, launches a canonical-root collection PTY through the UI, checks its repository-set environment, runs the deterministic Codex mock launcher, and verifies hierarchy/session persistence after reload. | Add edit/delete browser coverage for the atomic grouping API. |
...
| Overview tab | Partial | High | Unit coverage verifies that every live PTY appears without an open tab, attention and current-plan state render, and cards open the timeline or plan modal. No real-stack browser spec opens Overview yet. | Add an overview e2e scenario with two live sessions and verify attention ordering plus jumps to timeline and attached plan. |
| Published plans | Partial | High | Postgres-backed integration tests cover create/update/complete/history/app-state projection, the branch cycle (span anchors, depth-2 nesting, close refusal, return-to-parent), and guidance validation, live inheritance, and before/after audit history. Component tests cover creation, phase updates, branching, returning, guidance edits, rejected drafts, and ancestor refresh. | Add a real-stack browser scenario for sidebar creation, guidance editing and inherited reads, PTY attachment, phase completion, branch/return with the PTY following the work, Overview projection, and closed history. |
...
| Terminal integration surfaces | Covered | Medium | `04`, `06`, and `07` cover prompt injection, snapshot-on-attach, typed input echo, streamed output, resize handling, large-text and clipboard-image paste-as-file, reconnect, dead-session UI, and mock agent launch from a real shell session. Timeline prompt-bar behavior has unit coverage but no full e2e path yet. | Add right-click paste, selection-copy browser coverage, and a prompt-bar-to-agent e2e path if those interactions prove stable enough under Playwright. |
...
| Mobile shell | Covered | Medium | `05` seeds a terminal-only desktop preference, opens a session through the drawer, and verifies that mobile selects a visible timeline without creating a terminal tab. It also checks that display/peek hotkeys leave the desktop preference unchanged and that the shell fits the viewport. | Add mobile file/ref navigation and drawer interactions with multiple open sessions/tabs. |
