...
The broker and node containers both consume that value through normal Sulion
deploy secret injection. It is not forwarded into PTY shells. The control
process has no PTY credential file and never receives the broker master key.
